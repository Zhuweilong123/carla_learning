...
import motion_plan
from Controller import Vehicle_control
from global_path_plan import global_path_planner
from Sensors_detector_lib import Obstacle_detector
...
def motion_planning(conn):
    while 1:
        ...
        local_frenet_path_ = planner_utiles.sampling(match_point_list_[0], global_frenet_path_, back_length=10, forward_length=30)
        ...
        if len(possible_obs_) != 0 and possible_obs_[0][-1] <= 30:
            ...
            dp_path_s, dp_path_l = motion_plan.DP_algorithm(obs_s_list, obs_l_list,
                                                            ...
                                                            sampling_res=2)
            print("**motion planning time cost:", time.time() - start_time)
            l_min, l_max = motion_plan.cal_lmin_lmax(dp_path_s=dp_path_s, dp_path_l=dp_path_l,
                                                     obs_s_list=obs_s_list, obs_l_list=obs_l_list,
                                                     obs_length=5, obs_width=1.6)
            print("l_min", l_min)
            print("l_max", l_max)
            print("the length of quadratic planning:", len(dp_path_l))
            # 二次规划变量过多会导致计算延迟比较高，需要平衡二者之间的关系
            qp_path_l, qp_path_dl, qp_path_ddl = motion_plan.Quadratic_planning(l_min, l_max, dp_path_l,
                                                                                plan_start_l=begin_l_list[0],
                                                                                plan_start_dl=l_ds_list[0],
                                                                                plan_start_ddl=l_dds_list[0])

            print("qp_path_s", dp_path_s)
            print("qp_path_l", qp_path_l)
            # qp_path_l = list(2.5*np.array(qp_path_l))
            current_local_frenet_path_opt = motion_plan.frenet_2_x_y_theta_kappa(plan_start_s=begin_s_list[0],
                                                                                 plan_start_l=begin_l_list[0],
                                                                                 enriched_s_list=dp_path_s,
                                                                                 enriched_l_list=qp_path_l,
                                                                                 frenet_path_opt=local_frenet_path_opt_,
                                                                                 s_map=s_map)
        else:
            current_local_frenet_path_opt = local_frenet_path_opt_
        # 将重新规划得到的路径信息发送给主进程，让控制器进行轨迹跟踪
        conn.send((current_local_frenet_path_opt, match_point_list_))
        print("***motion planning time cost:", time.time() - start_time)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if count % 100 == 0:  # 这里表示控制器执行一千规划器执行一次
            ...
            # 调用子进程进行运动规划
            # 预测ts秒过后车辆的位置，以预测点作为规划起点
            pred_x, pred_y, pred_fi = planner_utiles.predict_block(model3_actor, ts=0.2)
            ...
            if count != 100:  # 第一个循环周期，因为有初始阶段规划好的局部路径，第二个周期的规划还未计算完成，一旦执行接收数据，会阻塞主进程
                cur_local_frenet_path_opt, match_point_list = conn1.recv()  # 新规划出的轨迹
                # print("local_frenet_path_opt", local_frenet_path_opt)
                """轨迹拼接
                思路比较简单，由于规划是在预测点进行的，对下个周期进行规划，因此当前周期的车辆运动结束点一定在预测点之前，
                找到上个规划周期轨迹中距离预测点最近的点，与新的规划路径进行拼接，保证轨迹的连续性
                """
                min_DIS = 10000
